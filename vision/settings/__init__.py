...
def get_settings_priority(priority):
    ...
    :attr:`~Vision.settings.SETTINGS_PRIORITIES` dictionary and returns its
...
class BaseSettings(MutableMapping):
    ...
    already an instance of :class:`~Vision.settings.BaseSettings`, in which
    case the existing priority levels will be kept).  If the ``priority``
    argument is a string, the priority name will be looked up in
    :attr:`~Vision.settings.SETTINGS_PRIORITIES`. Otherwise, a specific integer
    should be provided.

    Once the object is created, new settings can be loaded or updated with the
    :meth:`~Vision.settings.BaseSettings.set` method, and can be accessed with
    the square bracket notation of dictionaries, or with the
    :meth:`~Vision.settings.BaseSettings.get` method of the instance and its
    ...
    def getdict(self, name, default=None):
        ...
        :class:`~Vision.settings.BaseSettings` instance itself, it will be
        converted to a dictionary, containing all its current settings values
        as they would be returned by :meth:`~Vision.settings.BaseSettings.get`,
    ...
    def maxpriority(self):
        ...
        :attr:`~Vision.settings.SETTINGS_PRIORITIES` if there are no settings
        stored.
        """
        if len(self) > 0:
    ...
    def set(self, name, value, priority='project'):
        ...
        (through the :meth:`~Vision.crawler.Crawler.configure` method),
        ...
        :param priority: the priority of the setting. Should be a key of
            :attr:`~Vision.settings.SETTINGS_PRIORITIES` or an integer
    ...
    def setmodule(self, module, priority='project'):
        ...
        :meth:`~Vision.settings.BaseSettings.set` for every globally declared
    ...
    def update(self, values, priority='project'):
        ...
        This is a helper function that calls
        :meth:`~Vision.settings.BaseSettings.set` for every item of ``values``
        with the provided ``priority``.

        If ``values`` is a string, it is assumed to be JSON-encoded and parsed
        into a dict with ``json.loads()`` first. If it is a
        :class:`~Vision.settings.BaseSettings` instance, the per-key priorities
        will be used and the ``priority`` parameter ignored. This allows
        inserting/updating settings with different priorities with a single
        command.

        :param values: the settings names and values
        :type values: dict or string or :class:`~Vision.settings.BaseSettings`
...
class Settings(BaseSettings):
    """
    This object stores Vision settings for the configuration of internal
    components, and can be used for any further customization.

    It is a direct subclass and supports all methods of
    :class:`~Vision.settings.BaseSettings`. Additionally, after instantiation
